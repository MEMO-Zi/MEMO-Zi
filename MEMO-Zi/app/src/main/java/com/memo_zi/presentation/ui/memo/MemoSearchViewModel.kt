...
class MemoSearchViewModel @Inject constructor(
    ...
    init {
        _memoList.value = listOf(
            MemoSearchFeedItem.Title("투두리스트", 2), MemoSearchFeedItem.AllFeedItem(
                "오늘의 할일", "2023-01-30", "메모지 회의!"
            ), MemoSearchFeedItem.AllFeedItem(
                "오늘의 할일", "2023-01-31", "어려운것...서치뷰 만들기"
            ),

            MemoSearchFeedItem.Title("회의 일정", 1), MemoSearchFeedItem.AllFeedItem(
                "test1", "2023-01-30", "메모지 회의!"
            ), MemoSearchFeedItem.AllFeedItem(
                "test1 ", "2023-01-31", "어려운것...서치뷰 만들기"
            )
        )
    }
}
